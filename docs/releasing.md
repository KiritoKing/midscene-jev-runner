...
4. A person selects that existing tag in GitHub Releases, reviews the notes, marks `-beta.N` as a pre-release or stable `X.Y.Z` as a regular release, and clicks **Publish release**. This is the release approval and the only publication trigger. Saving a draft does not trigger npm publishing. The agent must not publish the GitHub Release.
